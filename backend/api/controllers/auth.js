...
exports.register=async(req,res,next)=>{
    try {
        ...
        if (!full_name || !email || !password || !validator.isEmail(email) || !passwordRegex.test(password)) return res.status(400).json({ status: false, message: 'Invalid Data' });
        const user = await userSchema.findOne({ email });
        if (user) return res.status(400).json({ status: false, message: 'Email already exists' });
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);
        const newUser = new userSchema({ _id: new mongoose.Types.ObjectId, full_name, email, password: hashedPassword })
        newUser.save();
